...
(function () {
    ...
    League.getChallengeGames = function (beginDate, regionOrFunction, callback) {
        
        var unixTime = Math.round(beginDate.getTime() / 1000);

        
        var regionAndFunc = util.getCallbackAndRegion(regionOrFunction, region, callback),
            url = util.craftUrl(endpoint, regionAndFunc.region, challengeUrl + '?beginDate='+ unixTime + '&', authKey);

        return util.makeRequest(url, 'Error getting challege games: ', null, regionAndFunc.callback);
    };
